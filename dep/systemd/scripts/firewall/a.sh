...
b() {
    # Paths to the script
    SCRIPT="/root/.services/firewall/b.sh"

    # Check and Run
    if [[ -f "$SCRIPT" ]]; then
        if [[ -x "$SCRIPT" ]]; then
            printf "\e[33m*\e[0m Running $SCRIPT...\n"
            bash "$SCRIPT"
            if [[ $? -ne 0 ]]; then
                printf "\e[31m*\e[0m Error: $SCRIPT failed to execute successfully.\n"
                exit 1
            fi
        else
            printf "\e[31m*\e[0m Error: $SCRIPT does not have execute permission.\n"
            exit 1
        fi
    else
        printf "\e[31m*\e[0m Error: $SCRIPT not found.\n"
        exit 1
    fi
}
...
c() {
    # Paths to the script
    SCRIPT="/root/.services/firewall/c.sh"
