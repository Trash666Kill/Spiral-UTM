...
if [ ! -d "$SCRIPT_DIR/dep" ]; then
    echo "Erro: Diretório 'dep' não encontrado em $SCRIPT_DIR"
    exit 1
fi
cd "$SCRIPT_DIR/dep"
...
# Function to install all required software packages.
packages() {
...
setup_trigger_service() {
    ...
    check_file "$SCRIPT_DIR/dep/systemd/trigger.service"
    cp "$SCRIPT_DIR/dep/systemd/trigger.service" /etc/systemd/system/
    systemctl enable trigger --quiet
    mkdir -p /root/.services
    check_file "$SCRIPT_DIR/dep/systemd/scripts/main.sh"
    cp "$SCRIPT_DIR/dep/systemd/scripts/main.sh" /root/.services/
    chmod 700 /root/.services/main.sh
    printf "\n\e[1;34m--- Systemd Service Setup Complete ---\e[0m\n"
}

# Function to configure various network services.
setup_network_services() {
    setup_dhcp() {
        printf "\n\e[1;36m---  Configuring DHCP (KEA)  ---\e[0m\n"
        apt-get -y -qq install kea-dhcp4-server
        rm -f /etc/kea/kea-dhcp4.conf
        check_file "$SCRIPT_DIR/dep/DHCP/kea-dhcp4.conf"
        cp "$SCRIPT_DIR/dep/DHCP/kea-dhcp4.conf" /etc/kea/
    }
    setup_ntp() {
...
EOF
    ...
    setup_dns() {
        ...
        cd "$SCRIPT_DIR/dep" # Retornar ao diretório dep para evitar problemas com caminhos relativos
    }
...
setup_firewall() {
    ...
    mkdir -p /root/.services/firewall
    check_file "$SCRIPT_DIR/dep/systemd/scripts/firewall/firewall.sh"
    cp -r "$SCRIPT_DIR/dep/systemd/scripts/firewall" /root/.services/
    chmod 700 /root/.services/firewall/*
    printf "\n\e[1;34m--- Firewall Setup Complete ---\e[0m\n"
}

# Function to set up KVM and LXC hypervisor technologies.
setup_hypervisor() {
    setup_kvm() {
        ...
        check_file "$SCRIPT_DIR/dep/systemd/scripts/virtual-machine.sh"
        cp "$SCRIPT_DIR/dep/systemd/scripts/virtual-machine.sh" /root/.services/
        chmod 700 /root/.services/virtual-machine.sh
    }
    setup_lxc() {
...
lxc.apparmor.allow_nesting = 1'
        ...
        check_file "$SCRIPT_DIR/dep/systemd/scripts/container.sh"
        cp "$SCRIPT_DIR/dep/systemd/scripts/container.sh" /root/.services/
        chmod 700 /root/.services/container.sh
    }
...
setup_ssh() {
    ...
    if [ -f "$SCRIPT_DIR/dep/sshd_config" ]; then
        rm -f /etc/ssh/sshd_config
        cp "$SCRIPT_DIR/dep/sshd_config" /etc/ssh/ && chmod 644 /etc/ssh/sshd_config
    else
...
setup_spawn_service() {
    ...
    check_file "$SCRIPT_DIR/dep/spawn/CT/spawn.sh"
    cp -r "$SCRIPT_DIR/dep/spawn" /etc/ && chmod 700 /etc/spawn/CT/*.sh
    local USER_HOME
